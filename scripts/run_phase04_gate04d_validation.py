#!/usr/bin/env python3
"""Gate 04D validation-only comparative dry run.

This entry point deliberately constructs validation datasets/loaders directly.
It never asks the generic train/validation/internal-test loader factory to build
an internal-test loader, and it refuses any config that opens internal-test
execution.
"""
...
def _load_config(path: Path) -> dict[str, Any]:
    ...
    if raw.get("execution_split") != "validation":
        raise ValueError("Gate04D is validation-only.")
    if bool(raw.get("internal_test_execution_allowed")):
        raise ValueError("Gate04D must not allow internal-test execution.")
    ...
    if not isinstance(manifests, dict) or set(manifests) != REQUIRED_MANIFEST_KEYS:
        raise ValueError(
            "Gate04D requires exactly isic2019/task3 authoritative manifest paths."
        )
...
def _validation_loader(
    ...
    *,
    ...
    if batch_size <= 0 or num_workers < 0 or prefetch_factor <= 0:
        raise ValueError("Invalid Gate04D loader settings.")
...
def _assert_unique_sample_ids(rows: list[dict[str, object]]) -> None:
    ...
    if len(sample_ids) != len(set(sample_ids)):
        raise ValueError("Paired validation export contains duplicate sample IDs.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("reports/phase04_controlled_comparative/gate04d_validation"),
    )
    ...
    # Validation-only datasets. No internal-test dataset object is constructed here.
    eval_transform = build_eval_transform()
    flat_dataset = ISIC2019HierarchicalDataset(
        isic_manifest, root, "validation", "flat_four_class", eval_transform
    )
    task1_dataset = ISIC2019HierarchicalDataset(
        isic_manifest, root, "validation", "stage_1", eval_transform
    )
    task2_dataset = ISIC2019HierarchicalDataset(
        isic_manifest, root, "validation", "stage_2", eval_transform
    )
    task3_dataset = EMBStage03Dataset(
        task3_manifest, root, "validation", eval_transform
    )

    flat_loader = _validation_loader(flat_dataset, **loader_kwargs)
    task1_loader = _validation_loader(task1_dataset, **loader_kwargs)
    task2_loader = _validation_loader(task2_dataset, **loader_kwargs)
    task3_loader = _validation_loader(task3_dataset, **loader_kwargs)

    raw_checkpoints = config["checkpoints"]
    specs = {name: _spec(name, raw_checkpoints[name]) for name in REQUIRED_CHECKPOINTS}

    # Load checkpoint payloads on CPU first so validation/benchmark GPU residency can
    ...
    if list(paired_rows[0]) != required_paired_columns:
        raise ValueError(
            "Paired export schema differs from the frozen Gate04D config: "
            f"expected {required_paired_columns}, got {list(paired_rows[0])}."
        )

    output_dir = args.output_dir if args.output_dir.is_absolute() else root / args.output_dir
    output_dir = output_dir.resolve()
    paired_path = output_dir / "paired_validation_predictions.csv"
    ...
    summary_path = output_dir / "gate04d_validation_summary.json"
    write_json(summary_path, payload)
    print(f"Gate04D validation-only dry run PASS: {summary_path}")
    print("internal_test_executed=false")
