#!/usr/bin/env python3
"""ITT-05: frozen seven-CNN consensus analysis on the locked HIBA external cohort.

This script performs no training, checkpoint loading, threshold tuning, or ISIC
internal-test recomputation. It consumes only the seven committed HIBA Flat
prediction CSVs and writes new ITT artifacts under
...
INPUT_TEMPLATE = Path(
    "results/paper/external_hiba/gate06f/{model}/paired_hiba_predictions.csv"
)
DEFAULT_OUTPUT = Path("results/extensions/itt2026_consensus/external_hiba")
EXPECTED_ROWS = 1232
EXPECTED_PATIENTS = 568
...
def main() -> int:
    ...
    if output.exists() and any(output.iterdir()):
        raise SystemExit(
            f"Refusing to overwrite existing ITT-05 output directory: {output}. "
            "Inspect/remove it manually only if a previous run was incomplete."
        )
    ...
    for model in FROZEN_MODELS:
        ...
        if not path.is_file():
            raise SystemExit(f"Missing frozen HIBA prediction file: {path}")
    ...
    summary = {
        "status": "PASS",
        "phase": "ITT-05",
        "cohort": "HIBA frozen external zero-shot",
        "sample_count": int(target.size),
        "patient_count": int(aligned["patient_id"].nunique()),
        "class_support": {
        ...
        "interpretation_boundary": (
            "HIBA is zero-shot. No threshold, model, or weight is selected or "
            "modified from this cohort."
        ),
    }
    ...
    _write_json(output / "itt05_hiba_results.json", summary)
    _write_json(
        output / "itt05_execution_manifest.json",
        {
            "phase": "ITT-05",
            "status": "PASS",
            ...
            "outputs": [
                ...
                "itt05_hiba_results.json",
                "itt05_execution_manifest.json",
            ],
